...
from frontend_server.services import display_disks_service
from frontend_server.utilities import disk_manager
from frontend_server.utilities import cache
from frontend_server.utilities import disk_util
from frontend_server.utilities import service_util
...
class InitService(base_service.BaseService):
    ...
    def before_setup(self, entry):
        ...
        if "scratch" not in self._args.keys():
            ...
            for disk_UUID in new_disks:
                if (
                    entry.application_context["available_disks"][disk_UUID]["volume_UUID"]
                    != common_UUID
                ):
                    raise RuntimeError(
    ...
    def after_existing_mount(self, entry):
        ...
        for disk_UUID, disk in self._volume["disks"].items():
            self._volume["disks"][disk_UUID]["level"] = int(disks_data[disk_num][0])
